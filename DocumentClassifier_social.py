### HP changed class FeatureInferencer(object) and class DocumentInferencer(object) in 20180413
...
class FeatureInferencer(object):
    match_checker = dict()       #{'pe': {0: {'historical', 'definite_negated_existence', 'others', ...}}}
    inference_map = dict()       #  {'pe': {0: 'negated_concept'}}
    rule_conclusion_types = []   # ['negated_concept']
    rule_source_types = []       # ['pe']
    ...
    def __init__(self, ruleFile, header_lines=0, delimiter=','):
        ...
        #[['negated_concept', 'pe', 'type1', 'type2', ..., 'type9']]
        ...
        rule_id = 0
        for rule in rules:                                       # id= 0
            conclusion_type = rule[0]                            # conclusion_type = 'negated_concept'
            source_type = rule[1]                                # source_type = 'pe'
            self.rule_conclusion_types.append(conclusion_type)   # rule_conclusion_types > append = 'negated_concept'
            self.rule_source_types.append(source_type)           # rule_source_types > append  = 'pe'
            condition_values = rule[2:]                          # condition_values =  'type1', 'type2', ..., 'type9'
            if source_type not in match_checker:
                match_checker[source_type] = dict()              
                inference_map[source_type] = dict()
            # changed in 2018 indent following 2 lines
            match_checker[source_type][rule_id] = set(condition_values) # match_checker['pe'][0] = set('type1', 'type2', ..., 'type9')
            inference_map[source_type][rule_id] = conclusion_type       # inference_map['pe'][0] = 'negated_concept'
            rule_id += 1
        pass

    def process(self, annotations, relations):
        matched_conclusion_types = ['ISOLATION']         # Modified 2018, change 'pe' to other positive type for other documents
        inference_map = self.inference_map        # {'pe': {0: 'negated_concept'}}
        match_checker = self.match_checker        # {'pe': {0: {'historical', 'definite_negated_existence', 'others', ...}}}
        sorted_modifiers = dict()
        annotations_idx = annotations.set_index('markup_id')                    # df with index T0, T1, T2, T3, T4, T5, T6, ...
        target_markups = annotations[annotations['vis_category'] == 'Target']   # df with target (here pe) T0, T2, T6...
        targets = set(target_markups['markup_id'].tolist())                     # list                     T0, T2, T6...
        ...
        for relation_id, relation in relations.iterrows():
            relation_type = relation['type']                                             # 'definite_negated_existence' 'indication' ...
            ...
            target_id = relation['arg2_id']                                              # T0, T1, ...
            ...
            if target_id not in sorted_modifiers:
                sorted_modifiers[target_id] = set()          #{'T0': set()} empty set i.e. {}
            sorted_modifiers[target_id].add(relation_type)  #{'T0': {'definite_negated_existence','indication'},'T1': set(),'T3':{'sth'}}
         
        for target_id, modifiers in sorted_modifiers.items():
            source_type = annotations_idx.loc[target_id, 'type']                         # pe pe
            for rule_id, modifiers_in_rule in match_checker[source_type].items():
                      # match_checker = {'pe': {0: {'historical', 'definite_negated_existence', 'others', ...}}}
                #if modifiers_in_rule == modifiers or modifiers_in_rule == set(''):       # false  # 2018 comment out
                # 2018 changed here
                if modifiers_in_rule.issubset(modifiers) or modifiers_in_rule == set(''):   # 2018 added  if has modifier or no modifier
                    matched_conclusion_types = list((inference_map[source_type][rule_id]))  # matched_conclusion_types ['negated_concept']
...
#2018        for source_type, matcher in match_checker.items():     #{'pe': {0: {'historical', 'definite_negated_existence', 'others', ...}}}
#2018            for rule_id, condition_values in matcher.items():  #       {0: {'historical', 'definite_negated_existence', 'others', ...}}
#2018                if len(condition_values) == 0:
#2018                    for anno in annotations:
#2018                        if anno['type'] == source_type:        # 'pe' = 'pe'
#2018                            matched_conclusion_types.append(inference_map[source_type][rule_id])

# here has some problems for targets without modifiers (all targets should have modifier)                            
        #for target_id in targets:                                           # 2018 comment out # left over targets(pe) without modifiers
        #    type = annotations.loc[annotations['markup_id'] == target_id, 'type'].iloc[0]    # append type of target without removed (pe)
        #    matched_conclusion_types.append(type)                           # 2018 comment out
...
class DocumentInferencer(object):
    rule_matchers = dict()           # {0: {'pe'}}
    doc_conclusions = []             # ['pe_doc']
    expected_evidence_types = set()  # {'pe'}
    default_conclusion = 'NEG_DOC'   # no_pe

    def __init__(self, ruleFile, header_lines=0, delimiter=','):
        rules = read_csv_rules(ruleFile, lower=True, header_lines=header_lines, delimiter=delimiter)
        # rules = [['pe_doc', 'pe'], ['no_pe']]
        #
        rule_id = 0
        for rule in rules:
            doc_type = rule[0]      # id=0, pe_doc or id=1, no_pe
            # if no evidence type required, this is the default document conclusion type
            # 2018 change if len(rules) == 1:     # no_pe                                                # 2018 change rules to rule
            if len(rule) == 1:    
                self.default_conclusion = doc_type  # default_conclusion = no_pe
                continue
            self.doc_conclusions.append(doc_type) # doc_conclusions=pe_doc, ##if more lines, here can be more doc_types #better use dict()
            matcher = set(rule[1:])                 # matcher = pe
            # save this for optimizing FeatureInferencer processing
            self.expected_evidence_types.update(rule[1:])   # expected_evidence_types = pe
            self.rule_matchers[rule_id] = matcher           # id = 0, rule_matchers = pe
            rule_id += 1
        pass

    def process(self, matched_conclusion_types):
        for rule_id, matcher in self.rule_matchers.items():     # id = 0 matcher = pe
            if matcher.issubset(matched_conclusion_types):      # if "pe" in feature_inferencer results 
                return self.doc_conclusions[rule_id]            # pe_doc   
